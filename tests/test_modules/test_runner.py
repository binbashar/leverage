...
def test_run_interactive_success(mock_runner, mocker):
    ...
    assert result == 0
    mock_subprocess.assert_called_once_with(["/usr/bin/test_binary", "arg1", "arg2"], env=os.environ.copy(), cwd=None)
...
def test_run_with_env_vars(mock_runner, mocker):
    ...
    mock_subprocess.assert_called_once_with(["/usr/bin/test_binary", "arg1"], env=expected_env, cwd=None)
...
def test_run_with_working_directory(mock_runner, tmp_path, mocker):
    ...
    mock_subprocess.assert_called_once_with(["/usr/bin/test_binary", "arg1"], env=os.environ.copy(), cwd=tmp_path)
...
def test_run_with_no_args_defaults_to_empty_list(mock_runner, mocker):
    ...
    mock_subprocess.assert_called_once_with(["/usr/bin/test_binary"], env=os.environ.copy(), cwd=None)
...
def test_run_with_instance_env_vars_only(mocker):
    ...
    mock_subprocess.assert_called_once_with(["/usr/bin/test", "arg1"], env=expected_env, cwd=None)
...
def test_instance_env_vars_preserved_across_multiple_runs(mocker):
    ...
    # First run
    runner.run("arg1")
    mock_subprocess.assert_called_with(["/usr/bin/test", "arg1"], env=expected_env, cwd=None)

    # Second run - instance env vars should still be present
    runner.run("arg2")
    assert mock_subprocess.call_count == 2
    mock_subprocess.assert_called_with(["/usr/bin/test", "arg2"], env=expected_env, cwd=None)
