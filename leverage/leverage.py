"""
    Leverage Reference Architecture Build Tool
"""
...
from .path import get_build_script
from .logging import get_logger
from .logging import _attach_build_handler
from ._parsing import _print_tasks
...
class AmbiguousTaskNameError(RuntimeError):
    pass
...
def build(args):
    ...
    try:
        build_script = get_build_script(filename=args.file)
...
def _prepare_tasks_to_run(module, input_tasks):
    ...
    Returns:
        list(tuples): List of tasks paired with their corresponding args and kwargs as provided by the user.
    """
    tasks = []
    for input_task in input_tasks:
        ...
        # NOTE: Consider a stricter kind of name matching such as `task.name == name`
        task = [task for task in module["tasks"] if task.name.startswith(name)]
        ...
        if len(task) > 1:
            ...
            raise AmbiguousTaskNameError(f"Conflicting matches `{', '.join(task_names)}` for given task `{name}`.")
